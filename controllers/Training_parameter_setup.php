...
class Training_parameter_setup extends MY_Controller
{
    ...
    {   
        ...
        $this->redirect($this->class_uri('ASF014'));
    }
    ...
    public function ASF014($stTraining = null, $sSts = null, $aSt = null)
    {
    ...
    public function viewTabFilter($tabID)
    {
        ...
        redirect($this->class_uri('ASF014'));
    }
    ...
    public function deleteTT()
    {
        ...
        if (!empty($codeTT)) {
            ...
            $delVal = $this->mdl->deleteTTVal($codeTT);
    ...
    /*_____________________
        UPDATE PROCESS
    _____________________*/
    // edit max application
    public function editMA()
    {
        $maxAppl = $this->post('maxAppl', true);
        
        // GET INFO FROM DB
        if (!empty($maxAppl)) {
            $data['max_appl'] = $maxAppl;
            $this->renderAjax($data);
        } else {
            echo 'Invalid Request.';
        }
    ...
    public function updateMA()
    {
        ...
        // form / input validation
        $rule = array('maximum_application' => 'required|is_natural_no_zero|max_length[2]');
        ...
        if ($status == 1) {
            // Begin Update Record
            $update = $this->mdl->saveUpdateMA($form);
    ...
    /*_____________________
        GET DETAILS
    _____________________*/
    public function trainingCategoryList($sttVar = null, $stsVar = null)
    {
        $sttP = $this->input->post('trStr', true);
        $stsP = $this->input->post('trSts', true);

        // training category default filter
        $sttDef = 'Y';
        $stsDef = 'Y';
        if (empty($sttVar)){
            $stt = $sttDef;
    ...
    public function deleteTSL()
    {
        ...
        if (!empty($tsl_code)) {
            ...
            // Check whether record related to another table
            $delVal = $this->mdl->getTSLDelVal($tsl_code);
    ...
    public function deleteTOL()
    {
        ...
        if (!empty($tol_code)) {
            // Check whether record related to another table
            $delVal = $this->mdl->getTOLDelVal($tol_code);
            if(empty($delVal)){
    ...
    public function deleteTPR()
    {
        ...
        if (!empty($tpr_code)) {
            // Check whether record related to another table
            $delVal = $this->mdl->getTPRDelVal($tpr_code);
            if(empty($delVal)){
    ...
    public function deleteTPS()
    {
        ...
        if (!empty($tps_code)) {
            // Check whether record related to another table
            $delVal = $this->mdl->getTPSDelVal($tps_code);
            if(empty($delVal)){
    ...
    public function deleteTAS()
    {
        ...
        if (!empty($tas_code)) {
            // Check whether record related to another table
            $delVal = $this->mdl->getTASDelVal($tas_code);
            if(empty($delVal)){
    ...
    /*_____________________
        GET DETAILS
    _____________________*/
    public function trainingAssessmentSetupView($stsVar = null)
    {
        $stsP = $this->input->post('aSts', true);
    ...
    public function deleteTASV()
    {
        ...
        if (!empty($tasv_type)) {
            // Check whether record related to another table
            $recTASV = $this->mdl->getTASVVerDetail($tasv_type);
    ...
    public function deleteTRT()
    {
        ...
        if (!empty($trt_code)) {
            // Check whether record related to another table
            $delVal = $this->mdl->getTRTDelVal($trt_code);
            if(empty($delVal)){
